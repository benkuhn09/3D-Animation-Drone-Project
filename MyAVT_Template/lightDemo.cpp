...
void renderSim(void) {
	...
	// SPOTLIGHTS 
	float spotLightWorld[NUMBER_SPOT_LIGHTS][4];
	float spotLightDirWorld[NUMBER_SPOT_LIGHTS][4];

	float yawRadSpots = -drone.dirAngle * 3.14159265f / 180.0f;

	// build world-space pos & dir 
	for (int i = 0; i < NUMBER_SPOT_LIGHTS; ++i) {
		float lx = spotLightPos[i][0];
		float ly = spotLightPos[i][1];
		float lz = spotLightPos[i][2];

		spotLightWorld[i][0] = drone.pos[0] + lx * cos(yawRadSpots) - lz * sin(yawRadSpots);
		spotLightWorld[i][1] = drone.pos[1] + ly;
		spotLightWorld[i][2] = drone.pos[2] + lx * sin(yawRadSpots) + lz * cos(yawRadSpots);
		spotLightWorld[i][3] = 1.0f;

		float dx = spotLightDir[i][0];
		float dy = spotLightDir[i][1];
		float dz = spotLightDir[i][2];

		float rx = dx * cos(yawRadSpots) - dz * sin(yawRadSpots);
		float rz = dx * sin(yawRadSpots) + dz * cos(yawRadSpots);
		float ry = dy;

		spotLightDirWorld[i][0] = rx;
		spotLightDirWorld[i][1] = ry;
		spotLightDirWorld[i][2] = rz;
		spotLightDirWorld[i][3] = 0.0f;
	}
	...
	for (int i = 0; i < NUMBER_SPOT_LIGHTS; ++i) {
		mu.multMatrixPoint(gmu::VIEW, spotLightWorld[i], spotEyePos[i]);
		mu.multMatrixPoint(gmu::VIEW, spotLightDirWorld[i], spotEyeDir[i]);
	...
	float cutoffAngleDeg = 15.0f;
	float spotCutOff = cosf(cutoffAngleDeg * 3.14159265f / 180.0f);
