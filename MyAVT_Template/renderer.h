...
public:
    ...
    //fonction pour la directional light 
    void setDirectionalLight(float* direction, float r, float g, float b, bool on);

    //fonction pour les point lights
    void setPointLights(const float lightPos[NUMBER_POINT_LIGHTS][4], const float lightColor[NUMBER_POINT_LIGHTS][3], bool lightsOn);

    void setSpotLights(const float spotPosEye[][4], const float spotDirEye[][4], const float spotColor[][3], bool spotOn, float spotCutOff);
